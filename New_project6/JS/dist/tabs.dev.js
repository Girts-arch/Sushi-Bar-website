...
var tabHeader = document.querySelectorAll('.tabHeader');
tabHeader.forEach(function (button) {
  button.onclick = changeTabHeader;
});

function changeTabHeader() {
  var tabNumber = parseInt(this.dataset.tab);
...
function changeTabSection(tabNumber) {
  var currentTab = document.querySelector('.show-tab');
  currentTab.classList.remove('show-tab');
  currentTab.classList.add('hide-tab');
  var newSection = document.getElementById('tabSection-' + tabNumber);
